...
Uint8List encodeAddress(String a) {
  Uint8List encoded = Uint8List.fromList(hex.decode(a));
...
Uint8List encodeFixedLengthList(List<dynamic> l, String type, int length) {
  ...
  if(isDynamicType(type)) {
    ...
    for(var i = 0; i < length; i++) {
      ...
      if(encodeType(type, l[i]) != null){
        contents.addAll(encodeType(type, l[i])!.cast<int>());
      }
  ...
  if(type.startsWith('(') && type.endsWith(')')) {
    ...
    for(var i = 0; i < subtypes.length; i++) {
      if(isDynamicType(subtypes[i])) {
        ...
        if(encodeType(subtypes[i], data[i]) != null){
          contents.addAll(encodeType(subtypes[i], data[i])!.cast<int>());
        }

      } else {
        if(encodeType(subtypes[i], data[i]) != null){
          headers.addAll(encodeType(subtypes[i], data[i])!.cast<int>());
        }
